...
class TestRefreshMetricData:
    def test_retrieves_metric(self, metric, mock_fetch_metric_data, mock_insert_metric_data):
        def assert_eager_load(metric, *args):
            assert hasattr(metric, '_load_balancer_cache'), \
                'load_balancer object should eager loaded before calling fetch_metric_data'
            assert hasattr(metric.load_balancer, '_credential_cache'), \
                'load_balancer.credential object should eager loaded before calling fetch_metric_data'
    ...
    def test_begins_loading_past_one_day(self, metric, mock_fetch_metric_data, mock_insert_metric_data,
                                         mock_datetime_now):
        refresh_metric_data(metric.id)
        args, kwargs = mock_fetch_metric_data.call_args
    ...
    def test_queries_back_in_time_if_succeeded(self, metric, mock_fetch_metric_data, mock_insert_metric_data,
                                               mock_datetime_now):
    ...
    def test_doesnot_back_in_time_if_insert_failed(self, metric, mock_fetch_metric_data, mock_insert_metric_data,
                                                   mock_datetime_now):
...
class TestScheduleAllElbRefreshes:
    ...
    def test_schedules_multiple_elbs(self, load_balancer, mock_refresh_elb_metrics, service, credential):
        from cloudwatch.models import LoadBalancer
        additional_lb = LoadBalancer.objects.create(service=service, region='aws-region-1', name='lb-name-2',
